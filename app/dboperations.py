...
class Menu:
    ...
    def check_duplicate(self, menu):
        duplicate = "SELECT food_name FROM menu WHERE food_name = '{}'".format(menu["name"])
...
class Order(Menu):
    ...
    def update_order_status(self, orderId, order_status):
        ...
        command = "UPDATE orders SET order_status='{}' WHERE order_id ='{}'".format(order_status,orderId)
        cursor.execute(command)
        return_order_status = "SELECT * FROM orders WHERE order_id = '{}'".format(orderId)
